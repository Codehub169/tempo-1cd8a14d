...
def _format_filesize(size_bytes: Optional[int]) -> str:
    ...
    if size_bytes < 1024: # Avoid log(0) or log(<1024) issues for small byte values
...
async def _extract_yt_dlp_info(url: str, ydl_opts: Dict) -> Dict[str, Any]:
    ...
    try:
        ...
        # Ensure 'proxy' is set. yt-dlp uses "" for no proxy.
        # If proxy is None from ydl_opts, it might mean use system proxy. We explicitly disable.
        if ydl_opts_processed.get("proxy") is None:
             ydl_opts_processed["proxy"] = "" # Explicitly disable proxy if it was None
        elif "proxy" not in ydl_opts_processed:
             ydl_opts_processed.setdefault("proxy", "") # Ensure proxy is set if not passed at all

        with yt_dlp.YoutubeDL(ydl_opts_processed) as ydl:
            info = await loop.run_in_executor(None, lambda: ydl.extract_info(url, download=not ydl_opts_processed.get('skip_download', True)))
            return info
...
async def download_media(url: str, format_id: str, media_type: str, filename_prefix: str) -> Tuple[str, str, str]:
    ...
    output_template_initial = os.path.join(temp_dir, '%(id)s.%(ext)s') 

    ydl_opts = {
        'format': format_id,
        'outtmpl': output_template_initial,
        'noplaylist': True,
        'quiet': True,
        'no_warnings': True,
        'skip_download': False,
    ...
    try:
        ...
        # No need to set outtmpl for just info extraction
        info_ydl_opts.pop('outtmpl', None) 
        info_dict_for_meta = await _extract_yt_dlp_info(url, info_ydl_opts)

        # Update outtmpl to use title for better on-disk filenames
        # yt-dlp handles sanitization of %(title)s and %(ext)s
        ...
        if info_dict.get('requested_downloads') and len(info_dict['requested_downloads']) > 0:
            ...
            # 'filename' is often pre-postprocessing or for archives.
